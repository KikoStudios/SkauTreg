...
export default function Home() {
  ...
  const trips = useQuery(api.trips.list, troops && troops.length > 0 ? { troopId: troops[0]._id } : "skip");
  const members = useQuery(api.members.list, troops && troops.length > 0 ? { troopId: troops[0]._id } : "skip");
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        <h1 style={{ fontSize: "1.5rem", fontWeight: "900", margin: 0 }}>Přehled</h1>
        <Link href="/troop">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", marginBottom: "2rem" }}>
        ...
        >
          ...
          <div style={{ fontSize: "0.85rem", color: "var(--text-muted)", fontWeight: "600", marginBottom: "0.75rem" }}>Vaše Oddíly</div>
          <div style={{ fontSize: "2.5rem", fontWeight: "900" }}>{troops?.length || 0}</div>
        </div>
        <div style={{
          backgroundColor: "white",
          border: "4px solid #000",
        ...
        >
          ...
          <div style={{ fontSize: "0.85rem", color: "var(--text-muted)", fontWeight: "600", marginBottom: "0.75rem" }}>Výpravy</div>
          <div style={{ fontSize: "2.5rem", fontWeight: "900" }}>{trips?.length || 0}</div>
        </div>
        <div style={{
          backgroundColor: "white",
          border: "4px solid #000",
        ...
        >
          <div style={{ fontSize: "1.5rem", marginBottom: "0.75rem" }}>👥</div>
          <div style={{ fontSize: "0.85rem", color: "var(--text-muted)", fontWeight: "600", marginBottom: "0.75rem" }}>Členové</div>
          <div style={{ fontSize: "2.5rem", fontWeight: "900" }}>{members?.length || 0}</div>
        </div>
      </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "4px solid #000", backgroundColor: "#86efac" }}>
                  ...
                  <th style={{ padding: "1rem", textAlign: "left", fontWeight: "700" }}>Stav</th>
                </tr>
              </thead>
              <tbody>
      ...
      {/* Help Center */}
      <div>
        <h2 style={{ fontSize: "1.125rem", fontWeight: "900", marginBottom: "1rem" }}>Nápověda & Tipy 💡</h2>
        ...
        {!showHelp ? (
          ...
          >
            ...
            <div style={{ fontSize: "1.1rem", fontWeight: "700" }}>Potřebujete pomoc?</div>
            <div style={{ fontSize: "0.9rem", color: "var(--text-main)", marginTop: "0.75rem" }}>Klikněte pro otevření hledání →</div>
          </div>
        ) : (
          <div style={{
            backgroundColor: "white",
              ...
              {filteredTopics.length > 0 ? (
                filteredTopics.map((topic, idx) => (
                  ...
                  >
                    ...
                    <h4 style={{ fontSize: "0.95rem", fontWeight: "700", marginBottom: "0.25rem" }}>{topic.title}</h4>
                    <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", margin: 0, lineHeight: "1.4" }}>{topic.description}</p>
                  </div>
                ))
              ) : (
